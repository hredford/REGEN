...
tiles = list.files("G:\\REGEN\\Plot_Level_Datasets\\PlotPatches_Norm", full.names = TRUE)
#plot(tiles, mapview = TRUE)
names = list.files("G:\\REGEN\\Plot_Level_Datasets\\PlotPatches_Norm", 
                   pattern = ".laz", 
                   full.names = FALSE)

base.folder = "G:\\REGEN"

output.folder = sprintf("%s\\Plot_Level_Products\\Patch_UnderstoryPointClouds", base.folder)

dir.create(output.folder) # Create a folder that contains the classified tiles
...
for (i in c(1,79)) {
  ...
  #plot(lazclip)
  #writeLAS(lazclip, sprintf("%s\\%s_lazclip.laz", output.folder, name))
  ...
  #maxZ = quantile(ttops$Z, probs = 0.25, na.rm = TRUE)
  ...
  if (length(ttops$treeID) > 0) { # the ttops variable must have more than 1 tree for this next aprt
    ...
    #plot(nostems)
    #writeLAS(nostems, sprintf("%s\\%s_nostems.laz", output.folder, name))
  ...
  #plot(notree)
  #writeLAS(notree, sprintf("%s\\%s_notree.laz", output.folder, name))
  ...
  writeLAS(nosnags, sprintf("%s\\%s_UnderstoyOnly.laz", output.folder, name))
