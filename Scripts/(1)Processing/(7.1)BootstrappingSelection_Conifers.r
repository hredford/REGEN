...
write.csv(top_sig, "G:\\YOSE_Regen_Analysis\\Runs\\April18_run_CURRENT\\Plot_Level_Datasets\\VariableVotes_Conifer.csv", row.names = FALSE)
...
top_sig_summary <- top_sig_summary[1:min(10, nrow(top_sig_summary)), ]
...
ggplot(plot_df, aes(x = value, y = sig_vars, color = metric, shape = metric)) +
  ...
  labs(
    ...
    color = " ",
    shape = " ",
    title = "Top Significant Conifer Variables: AIC, AUC & Kappa"
  ) +
...
regen = dplyr::select(train, c(success, ndirw, b11w, emin_90to99, iqr_90to99, b12w, linear_55to85, 
                               rumple_85to95, curv_TAO, b3s, ndirw))
...
for (i in c(1:length(regen[,1]))) {
  ...
  #
  glmmodel = glm(success~ ndirw + b12w + emin_90to99 + iqr_90to99+ linear_55to85 , data = df, family = binomial)
...
regen = dplyr::select(train, c(success, ndirw, emin_90to99, iqr_90to99, linear_55to85))
...
final_model <- train(
  success ~ ndirw + emin_90to99 + iqr_90to99 + linear_55to85,
  data = regen,
  method = "glm",
  family = binomial,
  metric = "ROC",
