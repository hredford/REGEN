...
write.csv(top_sig, "G:\\YOSE_Regen_Analysis\\Runs\\April18_run_CURRENT\\Plot_Level_Datasets\\VariableVotes_Oak.csv", row.names = FALSE)
...
sub_variable = variable_importance
...
top_sig_summary <- top_sig_summary[c(1:7,29,33, 72), ]
...
ggplot(plot_df, aes(x = value, y = sig_vars, color = metric, shape = metric)) +
  ...
  labs(
    ...
    color = " ",
    shape = " ",
    title = "Top Significant Oak Variables: AIC, AUC & Kappa"
  ) +
...
regen = dplyr::select(train, c(success, ndirw, ndirs, curv_85to95, iqr95_90to99, red, green, blue, b12w))
#regen = regen[c(2:9, 11:30),] curv_85to95, iqr95_90to99, green, b9w
...
for (i in c(1:length(regen[,1]))) {
  ...
  #
  glmmodel = glm(success~ ndirw + iqr95_90to99 + red + blue,  data = df, family = binomial)
...
regen = dplyr::select(train, c(success, ndirw , iqr95_90to99 , red , blue))
...
final_model <- train(
  success ~ ndirw + iqr95_90to99 + red + blue,
  data = regen,
  method = "glm",
  family = binomial,
  metric = "ROC",
...
write.csv(prediction_table, "G:\\YOSE_Regen_Analysis\\Runs\\April18_run_CURRENT\\Plot_Level_Datasets\\PredTable_Oak.csv", row.names = FALSE)
